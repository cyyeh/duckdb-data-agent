...
new feature, add design doc first: change MAX_TOTAL_SIZE_BYTES as env that can be set in backend and frontend needs to use api to fetch the value, default 500MB
...
fix issues of 
Fast refresh only works when a file only exports components. Use a new file to share constants or functions between component in all context related files in frontend

### Prompt 7

is it good practices to put xxxContext files in a new folder and usexxx files in another new folder
...
also refine readme based on new functionality(adjustable upload max size via backend api) and project folder structure
